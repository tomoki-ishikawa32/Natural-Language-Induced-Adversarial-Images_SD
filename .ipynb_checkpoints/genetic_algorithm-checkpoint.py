...
from crop_image import crop_image_to_quarters
...
class Fitness(object):
    ...
    def compute_fitness(self, individual: Individual, evaluator: PerformanceEvaluator, step, index, targetlabel,selectclass):
        ...
        for i in range(2):
            ...
            # Stable Diffusion API にプロンプトを送信してbase64画像を取得
            base64_image = send(individual.prompt(self.targetword))
            if base64_image is None:
                print("Image generation failed.")
                continue

            # base64画像を保存してPIL形式でロード
            download_image(base64_image, out_file)

            image_names = crop_image_to_quarters(out_file)
            if image_names is None:
                print("Cropping failed. Skipping this sample.")
                continue
